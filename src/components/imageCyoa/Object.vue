<template>
  <span>
    ...
    <v-row v-if="isEditModeOn">
      <v-card class="ma-1" width="100%" outlined>
        ...
        <v-col cols="12" class="pa-0">
          <v-text-field
            type="number"
            v-if="row.isWeightedRandom && row.isButtonRow && row.buttonRandom"
            placeholder="100"
            hide-details
            v-model="object.randomWeight"
            label="Random Weight"
            filled
          ></v-text-field>
        </v-col>
        ...
        <v-expansion-panels multiple accordion>
          <v-expansion-panel v-if="object.scores.length > 0">
            <v-expansion-panel-header
              v-html="$sanitize('Scores: ' + object.scores.length, sanitizeArg)"
            ></v-expansion-panel-header>
                ...
                <v-row class="pa-0">
                  <v-col cols="6" class="pa-0 d-flex justify-center">
                    <v-checkbox
                      ...
                      label="Show Score?"
                      class="justify-center shrink mr-2 mt-1"
                    ></v-checkbox>
                  </v-col>
          ...
          <v-expansion-panel v-if="object.addons.length > 0">
            <v-expansion-panel-header
              v-html="$sanitize('Addons: ' + object.addons.length, sanitizeArg)"
            ></v-expansion-panel-header>
            <v-expansion-panel-content>
          ...
          <v-expansion-panel v-if="object.requireds.length > 0">
            <v-expansion-panel-header
              v-html="$sanitize('Requirements: ' + object.requireds.length, sanitizeArg)"
            ></v-expansion-panel-header>
            <v-expansion-panel-content>
              <!-- Shows the requirements, allows the user to delete or change its id -->
              <v-row class="pa-0">
                <v-col
                  :cols="typeof required.requireds !== 'undefined' && required.requireds.length > 0 ? '12' : '6'"
                  class="pa-2"
          ...
          <v-expansion-panel v-if="typeof object.groups !== 'undefined' && object.groups.length > 0">
            <v-expansion-panel-header
              v-html="
                $sanitize('Groups: ' + (typeof object.groups !== 'undefined' ? object.groups.length : ''), sanitizeArg)
              "
            ></v-expansion-panel-header>
            <v-expansion-panel-content>
              <!-- Shows the requirements, allows the user to delete or change its id -->
          ...
          <v-expansion-panel>
            ...
            <v-expansion-panel-content>
              ...
              <v-checkbox
                class="mt-n2"
                hide-details
                v-model="object.cleanACtivatedOnSelect"
              ...
              <v-checkbox
                hide-details
                v-model="object.isSelectableMultiple"
                label="The choice can be selected multiple times"
              ></v-checkbox>
              <v-checkbox
                hide-details
                v-if="object.isSelectableMultiple"
                v-model="object.isMultipleUseVariable"
                label="Press this to use a simple variable instead of a Point-type?"
              ></v-checkbox>
              <p v-if="object.isSelectableMultiple && !object.isMultipleUseVariable">
                The point type used here should only be used for this choice, and it can be hidden by placing something
                in 'Id needed to activate' in Features -> Manage Points.
              </p>
              <v-select
                class="pa-1"
                hide-details
                v-if="object.isSelectableMultiple && !object.isMultipleUseVariable"
                :items="app.pointTypes"
                v-model="object.multipleScoreId"
                item-text="name"
                item-value="id"
                filled
              ...
              <v-text-field
                ...
                v-model="object.numMultipleTimesMinus"
                filled
              ></v-text-field>
              <v-text-field
                hide-details
                ...
                v-model="object.numMultipleTimesPluss"
                filled
              ></v-text-field>

              <v-checkbox
                hide-details
                v-model="object.isNotSelectable"
                label="Selecting this choice will be impossible"
              ></v-checkbox>

              <v-checkbox
                hide-details
                v-model="object.activateOtherChoice"
                label="Forces another choice active:"
              ></v-checkbox>
              <v-col v-if="object.activateOtherChoice" class="py-0"
                >Works badly if multiple of these have the same ID, or if the target has requirements attached. You can
                use comma to activate multiple (ID,ID,ID).</v-col
              >

              <v-text-field
                hide-details
                v-if="object.activateOtherChoice"
                label="Id of the choice that will be activated"
                v-model="object.activateThisChoice"
                filled
              ></v-text-field>

              <v-checkbox
                hide-details
                v-model="object.deactivateOtherChoice"
                label="Will make another choice unselected:"
              ></v-checkbox>

              <v-col v-if="object.deactivateOtherChoice" class="py-0"
                >Will be useful if the target has scores with requirements, use a Group Id to turn of multiple. You can
                use comma to deactivate multiple (ID,ID,ID).</v-col
              >

              <v-text-field
                hide-details
                v-if="object.deactivateOtherChoice"
                label="Id of the choice that will be deactivated"
                v-model="object.deactivateThisChoice"
                filled
              ></v-text-field>
              ...
              <v-checkbox
                hide-details
                v-model="object.multiplyPointtypeIsOn"
                label="Multiply Points when activated:"
              ></v-checkbox>
              <v-col v-if="object.multiplyPointtypeIsOn" class="pb-0"
                >Not to be used on choices with scores. Wont work if the Allowed Choices on the row is bigger than
                0.</v-col
              >
              <v-select
                class="pa-1"
                hide-details
                v-if="object.multiplyPointtypeIsOn"
                :items="app.pointTypes"
                v-model="object.pointTypeToMultiply"
                item-text="name"
                item-value="id"
                filled
              ...
              <v-text-field
                class="pa-1"
                hide-details
                v-if="object.multiplyPointtypeIsOn && !object.multiplyPointtypeIsId"
                label="Multiplied by X"
                v-model="object.multiplyWithThis"
                filled
              ></v-text-field>
              ...
              <v-checkbox
                ...
                hide-details
                v-if="object.multiplyPointtypeIsOn"
                v-model="object.multiplyPointtypeIsId"
              ...
              <v-checkbox
                hide-details
                v-model="object.dividePointtypeIsOn"
              ...
              <v-select
                class="pa-1"
                hide-details
                v-if="object.dividePointtypeIsOn"
                :items="app.pointTypes"
                v-model="object.pointTypeToDivide"
                item-text="name"
                item-value="id"
                filled
                label="Point Type"
              ...
              <v-text-field
                ...
                v-if="object.dividePointtypeIsOn"
                label="Divided by X"
                v-model="object.divideWithThis"
                filled
              ></v-text-field>

              <v-checkbox
                hide-details
                v-model="object.textfieldIsOn"
              ...
              <v-select
                class="pa-1"
                hide-details
                v-if="object.textfieldIsOn"
                :items="app.words"
                v-model="object.idOfTheTextfieldWord"
                item-text="id"
                item-value="id"
                filled
              ...
              <v-text-field
                ...
                v-model="object.wordChangeSelect"
                filled
              ></v-text-field>
              ...
              <v-text-field
                ...
                v-model="object.wordChangeDeselect"
                filled
              ></v-text-field>

              <v-checkbox
                hide-details
                v-model="object.isImageUpload"
                label="Player can upload a picture by pressing this choice."
              ></v-checkbox>

              <v-checkbox
                hide-details
                v-model="object.addToAllowChoice"
              ...
              <v-select
                class="pa-1"
                hide-details
                v-if="object.addToAllowChoice"
                :items="this.app.rows"
                v-model="object.idOfAllowChoice"
                item-text="id"
                item-value="id"
                filled
              ...
              <v-text-field
                ...
                v-model.number="object.numbAddToAllowChoice"
                filled
              ></v-text-field>
            </v-expansion-panel-content>
          </v-expansion-panel>
    ...
    <span
      ...
      v-else-if="checkRequireds(row)"
      @click="
        [
          row.isInfoRow || object.isNotSelectable || object.isSelectableMultiple || object.isImageUpload
  ...
  methods: {
    ...
    selectedOneMore(object) {
      ...
      if (object.isMultipleUseVariable) {
        object.multipleUseVariable = typeof object.multipleUseVariable === "undefined" ? 0 : object.multipleUseVariable;
