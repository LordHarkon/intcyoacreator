<template>
  <div style="text-align: center" :style="rowBody">
    <v-card class="my-n2">
      <v-toolbar v-if="row.isEditModeOn" class="grey lighten-3" dense flat>
        ...
        <v-tooltip v-for="button in rowIconButtons" :key="button.index" bottom open-delay="1000">
          <template v-slot:activator="{ on }">
            <v-btn icon v-on="on">
              <v-icon @click="currentComponent = button.component">{{ button.icon }}</v-icon>
            </v-btn>
          </template>
        ...
        <v-row class="pa-2 ma-2">
          <v-col class="col-md-2 pa-2">
            ...
            <v-col class="pa-0" v-else>
              ...
              <v-btn class="mb-2" @click="currentComponent = 'appButtonSettings'" style="color: black"
                >Open Button Settings</v-btn
              >
            </v-col>
          </v-col>
          ...
          <v-col class="col-md-2 pa-2">
            ...
            <v-text-field
              ...
              v-model.number="row.allowedChoices"
          ...
          <v-col class="col-md-2 pa-2">
            ...
            <v-text-field
              ...
              v-model.number="row.currentChoices"
              placeholder="Placeholder"
              filled
            ></v-text-field>
          </v-col>
          ...
          <v-col class="col-md-4 pa-2">
            <v-textarea hide-details filled rows="8" label="Row Text" v-model="row.titleText"></v-textarea> </v-col
          ><v-row cols="12">
            <v-col>
              <v-switch
                class="mt-n2 mb-n6 ms-3"
                v-model="row.deselectChoices"
            ...
            <v-col>
              <v-switch
                v-if="row.isResultRow"
                class="mt-n2 mb-n6 ms-3"
                v-model="row.choicesShareTemplate"
            ...
            <v-col>
              <v-switch
                v-if="row.isResultRow"
                class="mt-n2 mb-n6 ms-3"
                v-model="row.resultShowRowTitle"
    ...
    <span>
      ...
      <div
        :style="row.title != '' && row.text != '' ? rowBackground : ''"
    ...
    <!-- TODO: by hooshu: check if the rowWasChange event works like this -->
    <component
      :is="currentComponent"
      :row="row"
      @rowWasChanged="updateRow($event)"
...
export default {
  ...
  methods: {
    ...
    createNewObject() {
      ...
      this.updateRow(modifiedRow);
    },
    buttonActivate() {
      if (this.row.btnPointAddon && this.row.buttonTypeRadio == "sumaddon") {
        //If Random
        let random = Math.floor(Math.random() * (this.row.randomMax - this.row.randomMin) + this.row.randomMin);
